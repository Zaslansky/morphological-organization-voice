...
##################################
##
## THE MORPHOLOGICAL ORGANIZATION OF VOICE
##
## Matthew Zaslansky
## v. 2.0.1
## version date: 31 January 2025
##
## Based on the formatting of Sims & Parker 2019.
##
...
parameters.fnc = function(data3, adjMatrix){
	...
	typeFreq <- data3[,2]  #Type frequency is in column 2
	size = log(as.numeric(as.character(typeFreq))+1) * 1.5	#Plot size of nodes -- weighted by log type frequency of class
...
# Color of edges
#If the language plat has only one class, then there are no edges and thus no weight. Assign color black just to keep script from throwing an error.
	if(nrow(data3) == 1){
		color = "black"
	}else{
...
#colorByModule is boolean: should nodes be colored according to their groups (modules)?

#trim is boolean: should weak edges in the graph be trimmed, or not?
...
#Allowing plotting of graphs which have only one node (one inflection class), and therefore no edges and no edge weight:
		if(nrow(plat)==1){
			maxWeight = 0
			E(plot)$edgeColor <- "black"
			E(plot)$rescale = 0
...
## There is some customization in this script to particular languages, but the customized bits will be ignored by the script in the general case 
## and the script should run successfully without any customization (or taking out/modifying these parts).
...
### BEGIN CUSTOMIZATION -- but can be ignored.
...
### BEGIN MORE CUSTOMIZATION -- but can again be ignored.
...
#Only graph degree x edge weight if the language plat has more than one class
	if(nrow(plat) > 1){
		table_temp <- table(as.numeric(as.character(networkProperties$summedWeight)))
		countNodes <- as.vector(table_temp) * as.numeric(labels(table_temp)[[1]])
#In the input file, each node is a row, so nrow(networkProperties) = number of nodes in the network
		...
		degreeWeight <- as.numeric(names(table_temp)) 
...
networkPropertyScatters.fnc = function(language, plat, degree_data){
	...
	log_freq = log(as.numeric(as.character(plat[,2]))) #Type frequency is in column 2
...
#Only do this if the plat has more than one class. Modularity is trivial if there is only one class.
	if(nrow(plat) > 1){
...
#Order the data frame
		...
		cat(paste0("Plat ", language, " has only one class. Modularity not calculated.\n"))
	}
}
...
#Plot results of entropy difference calculations (pre-calculated). 
...
## Graph t-values for main effect node-level factors (node size, degree, summed edge weight (residualized), local clustering coefficient) 
## as predictors of entropy difference in multiple linear regression models (separately fit for each plat). This function is called from 
## comparison.fnc() in main.R, but the call is commented out by default, because the input file below must be hand-generated..

graphTVals.fnc = function(){

## WARNING! This input file needs to be created by hand. It contains t-values from fitted regression models for node size (type frequency), 
## degree, local clustering coefficient, and edge weight as main effect predictors (with entropy difference as the dependent variable). Since 
## the models need to be fitted by hand, it doesn't make sense to auto-generate this file. Models independently need to be fit in 
## complexityComponentsGraph.fnc() in complexityIncrease.R, so the values needed for this file can be determined by inspecting the model summary. 
## A sample input file is available with this code.
...
#Only calculate for languages with more than one class
		if(nrow(temp_plat) <= 1 ){
			cat("Plat ", filenames[i], "has only one class. Skipping Monte Carlo simulation.\n")
		}else{
...
#
#	plot <- ggplot(data, aes(x = as.numeric(as.character(meanPathLength)), y = as.numeric(as.character(clusterCoef)))) + geom_point(aes(group = factor(simulated), color = factor(simulated)), size=3) + geom_text_repel(aes(x = as.numeric(as.character(meanPathLength)), y = as.numeric(as.character(clusterCoef)), group = factor(simulated), color = factor(simulated), label = lang), size = 6.5) + xlim(1,xmax) + ylim(0,1) + theme_bw() + scale_color_manual(values = c("blue", "black"), breaks=c("0", "1")) + theme(legend.position="none") + xlab("Mean Shortest Path Length\n(unweighted)") + ylab("Global Clustering Coefficient") + theme(axis.text=element_text(size=14), axis.title=element_text(size=16, face="bold"))
#
